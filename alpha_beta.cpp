...
int minimax_alpha_beta(Node *root, bool isMax,int alpha,int beta){
    ...
    if (isMax){                                 //If Max's turn
        ...
        for(vector<Node *>::size_type i = 0; i != temp_child.size(); i++){
                int temp = minimax_alpha_beta(temp_child[i],false,alpha,beta);
                best = max(best, temp);
        ...
        int best = +100;
        for(vector<Node *>::size_type i = 0; i != temp_child.size(); i++){
                int temp = minimax_alpha_beta(temp_child[i],true,alpha,beta);
                best = min(best, temp);
                beta = min(best,beta);
                if(beta<=alpha) break;
        }
...
int main(int argc, char* argv[]){
    ...
    int result = minimax_alpha_beta(root,true,-1,1);
